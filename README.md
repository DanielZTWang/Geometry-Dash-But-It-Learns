...
3. In **Windows PowerShell**, run **"winget install GeodeSDK.GeodeCLI"** and **"winget install Python.Python.3.14"**
4. In **Windows PowerShell**, run **"geode sdk install"** and then **"geode sdk install-binaries"**
...
7. Run **"geode build" (Note: You may have to change the Geode version in mod.json in PercentageReader to match your Geode version if build fails)**
8. In **Windows PowerShell, cd** into the location of your **Geometry-Dash-But-It-Learns folder**
9. Run **"python -m pip install -r requirements"**
10. In **Geometry Dash settings**, in the **Gameplay** section check **Enable Quick Keys**.
11. In **Windows PowerShell, cd** into the location of your **Geometry-Dash-But-It-Learns folder**.
12. Run **"python src/genetic_algorithm_trainer.py"** or **"python src/run_best_genome.py"** depending on what you are trying to do.
